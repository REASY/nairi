...
You are helping the user reverse engineer Android APK files using apktool for security analysis, vulnerability
discovery, and understanding app internals.

## Tool Overview

Apktool is a tool for reverse engineering Android APK files. It can decode resources to nearly original form and rebuild
them after modifications. It's essential for:
...
Apktool works well within the NAIRI static analysis agent workflow:

1. **APK → Network Analysis**:
    - Extract API endpoints from resources/manifest.
    - Note domains for the Runtime MITM phase to intercept.

2. **APK → Credential Discovery**:
    - Find hardcoded credentials in resources (`res/values/strings.xml`).
    - Report findings in the final NAIRI analysis report.

3. **APK → Code Analysis**:
    - Extract smali code with apktool to understand native library loading.
    - Cross-reference with `jadx` or `ghidra` for deeper analysis.
